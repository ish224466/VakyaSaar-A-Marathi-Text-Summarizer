...
const ConversationListItem: React.FC<ConversationListItemProps> = ({
  ...
  if (isSelected) {
    return (
      <li key={convo.id} className="relative z-15 mb-2" style={{opacity: 1, height: "auto"}}>
        <div
          role="button"
          className={`relative flex py-3 px-3 items-center gap-3 rounded-md bg-slate-800 text-white dark:bg-gray-850 cursor-pointer break-all pr-14 group`}
        >
    ...
    return (
      <li key={convo.id} className="relative z-15 mb-2" style={{opacity: 1, height: "auto"}}>
        <button
          ...
          type="button"
          className="relative flex w-full py-3 px-3 items-center gap-3 dark:bg-gray-900 bg-slate-600 hover:bg-slate-500 dark:hover:bg-gray-850 rounded-md text-white cursor-pointer break-all"
        >
          <ChatBubbleLeftIcon {...iconProps} />
          <div
