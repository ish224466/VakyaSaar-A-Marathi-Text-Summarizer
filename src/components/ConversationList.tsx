  ...
  return (
    <div className="conversation-list-container">
      <div id="conversation-search" className="flex flex-row items-center mb-2 relative">
        <input
          id="searchInput"
          className="grow rounded-md border text-gray-100 dark:text-gray-100 bg-slate-800 dark:bg-gray-900 dark:border-white/20 px-2 py-1"
          type="text"
          autoComplete="off"
        ...
        >
          <MagnifyingGlassIcon
              style={{height: 23, width: 18, color: userSettings.theme === 'dark' ? '#FFFFFF' : '#fdfdfdff' }}
              {...iconProps} />
        </button>
        {/* {
                    showSearchOptions && (
                  ...
                  if ("marker" in convo) {
                    return (
                      <li key={`marker-${index}`}
                        ...
                        <h3
                          className="h-9 pb-2 pt-3 px-3 text-xs text-gray-400 font-medium text-ellipsis overflow-hidden dark:bg-gray-800" >
                          {convo.title}
                        </h3>
                      </li>
                    );
