...
const MainPage: React.FC<MainPageProps> = ({className, isSidebarCollapsed, toggleSidebarCollapse}) => {
  ...
  const callApp = (message: string, fileDataRef: FileDataRef[]) => {
    ...
    // Add user message and then create an assistant placeholder that will be
    // replaced once the summarizer returns. We also keep the existing send pipeline.
    addMessage(Role.User, MessageType.Normal, message, fileDataRef, sendMessage);

    (async () => {
      // create a unique id for placeholder
  const placeholderId = Date.now();
      ...
      // animate dots 1..3 every 500ms
      let dots = 1;
      const interval = setInterval(() => {
        setMessages(prev => prev.map(m => {
          if (m.id === placeholderId) {
            return { ...m, content: 'Summarizing' + '.'.repeat(dots) };
          }
          return m;
        }));
        dots = dots % 3 + 1;
    ...
    <div className={`${className} overflow-hidden w-full h-full relative flex z-0 dark:bg-gray-900`}>
      <div className="flex flex-col items-stretch w-full h-full">
        ...
        >
          ...
          <div className="absolute top-3 right-2 z-50 flex gap-2">
            <button
              onClick={handleImportConversation}
              className="px-4 py-2 bg-blue-400 text-sm text-black rounded-md hover:bg-blue-600"
            >
              {t('Import Chat')}
            </button>
            {conversation && (
              <button
                ...
                className="px-4 py-2 mr-2 bg-blue-900 text-sm text-white rounded-md hover:bg-blue-600"
              >
                {t('Export Chat')}
              </button>
            )}
