...
const ChatBlock: React.FC<Props> = ({block, loading, isLastBlock}) => {
  ...
  return (
      <div key={`chat-block-${block.id}`}
           className={`group w-full text-gray-800 dark:text-gray-100  border-black/10 rounded-lg pb-2 mx-3 dark:border-gray-900/50 
           `}>
              
    <div
      className={`w-4/5 text-base md:max-w-xl lg:max-w-xl xl:max-w-3xl 2xl:max-w-3xl 3xl:max-w-4xl 4xl:max-w-5xl p-2 flex lg:px-0 flex-col ${block.role === 'user' ? 'ml-auto' : 'mr-auto'}`}>
          <div className={`w-full flex p-3 pt-4 pb-3 rounded-xl ${block.role === 'assistant' ? 'bg-white dark:bg-gray-900 dark:hover:bg-gray-850' : 'bg-gray-200 dark:bg-gray-850 dark:hover:bg-gray-800 flex-row-reverse'}`}>
          ...
          `}>
            ...
            <div className="relative flex w-[calc(100%-50px)] flex-col gap-1 md:gap-3 lg:w-full ml-2">
              <div id={`message-block-${block.id}`} className="flex grow flex-col gap-3 pt-1"
                   style={errorStyles}>
                <div
